<?php
/*
 *      SwiftCloud Dashboard
 */
if (!function_exists('swift_dashboard_cb')) {

    function swift_dashboard_cb() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', SWIFTCLOUD__PLUGIN_URL . 'admin/js/jstz.min.js', '', '', true);
        wp_enqueue_style('swift-dashboard', SWIFTCLOUD__PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard-script', SWIFTCLOUD__PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);

        $subscribed = get_option(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <?php } ?>
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="swiftcloud_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_swiftcloud_dashboard_subscribe">
                    ...
                    <input type="hidden" name="formid" value="641" id="formid" />
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('https://SwiftCRM.Com/support/tag/inbound/feed'); //Change here
                                $maxitems = 0;
                                ...
                                <?php if ($maxitems == 0) : ?>
                                    <div class="col-dashboard-item">
                                        <p><?php _e('No items', 'swiftcloud'); ?></p>
                                    </div>
                                <?php else : ?>
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function swiftcloud_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function swiftcloud_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function swiftcloud_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str(sanitize_text_field($_POST['data']), $subscribe_form_data);
            $subscribe_form_data['referer'] = home_url();

            update_option(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(SWIFTCLOUD_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
